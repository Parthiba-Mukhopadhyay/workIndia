...
const Marketplace = () => {
  ...
  return (
    <div>
      ...
      <div className={`container mx-auto p-4 mt-20 ${layout === 'column' ? 'grid grid-cols-1' : 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'} gap-6`}>
          ...
          filteredItems.map((item, index) => (
            <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition duration-300">
              <img
                src={smallImage}
                alt={item.item}
                className={`w-full h-48 object-cover ${layout === 'column' ? 'block' : 'hidden'}`}
              />
              <img
                src={mediumImage}
                alt={item.item}
                className={`w-full h-48 object-cover ${layout === 'column' ? 'hidden' : 'block'}`}
              />
              <div className="p-4">
      ...
      <BottomComponent onLayoutChange={handleLayoutChange} />
    </div>
  );
};
